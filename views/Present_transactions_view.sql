...
dividend_selection AS (
  SELECT
    ...
    COALESCE(
      SAFE_DIVIDE(
        Transaction_price * Currency_close,
...
dividend_sum AS (
  SELECT
    ...
    SUM(Transaction_value_pln)                      AS dividend_sum,
    AVG(dividend_ratio_pct) *
          MAX(dividend_frequency)                   AS avg_dividend_ratio_per_ticker_pct
...
present_instruments_plus_present_indicators AS (
  SELECT
    ...
    inst_typ.Instrument_type_main                         AS Instrument_type_main,
    ...
    piv.ticker_present_amount * daily.Close * inst.unit   AS ticker_present_value,
    ...
    SAFE_DIVIDE(
      ...
      * inst.unit,
      ...
      OVER(PARTITION BY piv.Project_id))                  AS share_of_portfolio,
    SAFE_DIVIDE(
      piv.ticker_present_amount
      * daily.Close
      * inst.unit,
    ...
    CASE
      WHEN piv.max_age_of_instrument > 120
        THEN SAFE_DIVIDE(
          365 * (SAFE_DIVIDE(
            piv.ticker_present_amount * daily.Close * inst.unit,
            piv.ticker_buy_value) - 1),
            piv.max_age_of_instrument
        )
      ELSE 0
    ...
    CASE
      ...
      THEN COALESCE(
        SAFE_DIVIDE(
          365 * (SAFE_DIVIDE(
            piv.ticker_present_amount * daily.Close * inst.unit + dividend_sum,
            piv.ticker_buy_value) - 1),
            piv.max_age_of_instrument
        ),
        365 * SAFE_DIVIDE(
          SAFE_DIVIDE(piv.ticker_present_amount * daily.Close * inst.unit, piv.ticker_buy_value) - 1,
          piv.max_age_of_instrument)
      )
      ELSE 0
      END                                                 AS yearly_rate_of_return_incl_div,
    ticker_present_amount * (daily.Close * inst.unit - piv.ticker_average_close)
                                                          AS profit,
  IFNULL(
    div_sum.dividend_sum + (piv.ticker_present_amount * (daily.Close * inst.unit - piv.ticker_average_close)),
    piv.ticker_present_amount * (daily.Close * inst.unit - piv.ticker_average_close)
  )                                                       AS profit_incl_div,
    IFNULL(div_sum.avg_dividend_ratio_per_ticker_pct, 0)  AS avg_dividend_ratio_per_ticker_pct
...
SELECT
  *,
  share_of_portfolio * yearly_rate_of_return_incl_div                     AS yearly_rate_of_return_incl_div_weighted,
  share_of_portfolio_per_instrument_type * yearly_rate_of_return_incl_div AS yearly_rate_of_return_incl_div_weighted_per_instrument_type
